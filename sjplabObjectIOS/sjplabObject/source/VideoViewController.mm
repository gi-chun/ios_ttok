...
//    cvtColor(image, cmt_im_gray, CV_BGR2GRAY);
    ...
    if(isObjectSelected){
        ...
        if(firstStep){
            ...
            r.x = r.x * resize_size.width / kScreenBoundsWidth;
            r.y = r.y * resize_size.height / kScreenBoundsHeight;
            r.width = r.width * resize_size.width / kScreenBoundsWidth;
...
//                cvLine(img, cvPoint(center.x-2, center.y+2), cvPoint(center.x+2, center.y-2), rectangleColor, 2);
        ...
        if(cmt.hasResult && main->cmt_tracking){
            ...
            //cvRectangle(img, topleft, bottomRight, Scalar(0,0,255), 1, 4);
...
//            cvCircle(img,cv::Point(x,y),10,Scalar(0,255,0),1);
//            cvLine(img,cv::Point(x,y),cv::Point(x,y-15),Scalar(0,255,0),1);
//            cvLine(img,cv::Point(x,y),cv::Point(x,y+15),Scalar(0,255,0),1);
//            cvLine(img,cv::Point(x,y),cv::Point(x-15,y),Scalar(0,255,0),1);
//            cvLine(img,cv::Point(x,y),cv::Point(x+15,y),Scalar(0,255,0),1);
